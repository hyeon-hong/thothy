  ...
  useEffect(() => {
    ...
    const fetchPricingPolicies = async () => {
      ...
      try {
        ...
        // Filter and sort the pricing policies
        const filteredPolicies = (data || []).filter(
          (policy: PricingPolicy) => policy.visible
        );
          ...
          {pricingLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {pricingPolicies.map((policy) => (
                <div
                  key={policy.id}
                  className="bg-white p-6 rounded-lg border border-gray-200 flex flex-col h-full w-full md:w-auto pricing-card"
                >
                  <h3 className="text-xl font-semibold mb-4">{policy.name}</h3>
                  {policy.name === "Custom" || policy.name === "Enterprise" ? (
                    <div className="mb-4 text-lg font-bold">Custom Price</div>
                  ) : policy.monthly_price > 0 ? (
                    <div className="mb-4 text-lg font-bold">
                      ${policy.monthly_price.toFixed(2)}/month
                    </div>
                  ) : (
                    <div className="mb-4 text-lg font-bold">Free</div>
                  )}
                  <div className="space-y-2 text-gray-600 flex-grow">
                    {formatDescription(policy.description).map((item) => (
                      <p key={`${policy.id}-${item}`}>• {item}</p>
                    ))}
                  </div>
                  <Button
