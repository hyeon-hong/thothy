...
# Use a type alias to avoid circular import
...
async def process_pdf(pdf_path, query, client=None):
    ...
    try:
        if client:
            ...
            return get_pdf_retrieval_ans_from_assistant(
                client, pdf_path, query
            )
...
async def click(state: AgentState):
    ...
    if pdf_files:
        ...
        if "input" in state:
            pdf_observation = await process_pdf(
                pdf_path,
                state["input"],
                client
            )
...
async def extract_information(state: AgentState):
    # - Extract Information: [information you want to extract]
    # Takes the current page and a query, and extracts relevant information
    page = state["page"]
    input_arg = state["prediction"]["args"]

    # Get page content
    content = await page.content()

    # Default behavior is to return the current URL and title
    title = await page.title()
    url = page.url

    # If there's a query, try to extract specific information
    if input_arg and len(input_arg) > 0:
        query = input_arg[0]
        client = state.get("openai_client")

        if client:
            # Create a system prompt that instructs the model to extract
            # the requested information
            system_prompt = (
                "You are an AI assistant that extracts specific "
                "information from web pages. Based on the HTML content "
                "provided and the user's query, extract only the "
                "relevant information. Be concise and accurate."
            )

            # Create user prompt with the query and HTML content
            ...
            return (
                "No OpenAI client provided for information extraction.\n"
                f"URL: {url}\nTitle: {title}"
            )

    # Default response with URL and title
    return f"URL: {url}\nTitle: {title}"
